(() => {
  ...
  function timelineForRoom(roomCode, date) {
    const opening = 420, closing = 1290, merged = [];
  ...
  function renderAvailability() {
    ...
    elements.availabilityBoard.innerHTML = rooms.map((room) => `<article class="availability-room"><h3>Aula ${escapeHtml(room.code)}</h3><div class="availability-slots">${timelineForRoom(room.code, elements.boardDate.value).map((slot) => `<span class="availability-slot ${slot.free ? 'is-free' : 'is-busy'}" title="${escapeHtml(slot.label)}">${minutesToTime(slot.start)}–${minutesToTime(slot.end)} · ${slot.free ? 'Disponible' : 'Ocupado'}</span>`).join('')}</div></article>`).join('');
  }
  ...
  async function createUser(event) {
    ...
    const form = new FormData(elements.createUserForm), email = String(form.get('email')).trim().toLowerCase();
    if (!/^[a-z0-9-]+\.[a-z0-9-]+\.[a-z0-9-]+@una\.cr$/.test(email)) { showMessage('El correo debe tener el formato nombre.apellido.apellido@una.cr.', 'error'); setBusy(button, false); return; }
    try { const { data, error } = await state.client.functions.invoke('admin-create-user', { body: { fullName: String(form.get('name')).trim(), email, password: String(form.get('password')), role: String(form.get('role')) } }); if (error) throw error; if (!data?.ok) throw new Error(data?.error || 'No fue posible crear la cuenta.'); elements.createUserForm.reset(); showMessage('Cuenta creada correctamente.'); }
  ...
  async function uploadSchedule() {
    clearMessage(); const file = elements.scheduleFile.files[0]; if (!file) return showMessage('Selecciona primero el archivo CSV.', 'error');
    setBusy(elements.uploadScheduleButton, true, 'Cargando…');
    try { const entries = parseCsv(await file.text()); const { data, error } = await state.client.rpc('admin_replace_fixed_occupancies', { p_entries: entries }); if (error) throw error; await reloadAll(); showMessage(`Ocupación académica cargada: ${data} registros. Las reservas permanecen cerradas.`); }
  ...
  function bindEvents() {
    ...
    elements.scheduleFile.addEventListener('change', () => { const label = document.querySelector('label[for="scheduleFile"]'); label.textContent = elements.scheduleFile.files[0]?.name || 'Seleccionar CSV'; label.classList.toggle('has-file', Boolean(elements.scheduleFile.files[0])); });
    ...
    document.addEventListener('click', (event) => { const button = event.target.closest('[data-cancel-reservation]'); if (button) cancelReservation(button.dataset.cancelReservation); });
  }
